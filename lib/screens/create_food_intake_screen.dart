...
import '_edit_food_intake_form.dart';

class CreateFoodIntakeScreen extends StatefulWidget {
  final ProfileModel profile;

  CreateFoodIntakeScreen(this.profile);

  @override
  _CreateFoodIntakeScreenState createState() => _CreateFoodIntakeScreenState();
}

class _CreateFoodIntakeScreenState extends State<CreateFoodIntakeScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Create food intake', style: TextStyle(fontSize: 16)),
        actions: [
          IconButton(
            icon: Icon(
              Icons.check,
            ...
            onPressed: () {
              ...
              BlocProvider.of<HomeBloc>(context).add(CreateProductEvent(
                ...
                description: descriptionController.text.length > 0
                    ...
                    : null,
                barcode: barcodeController.text.length == 0
                    ? null
                    : int.parse(barcodeController.text),
                calorieContent: calorieContentController.text.length > 0
                    ? double.parse(calorieContentController.text)
                    : null,
                proteins: proteinsController.text.length > 0
                    ? double.parse(proteinsController.text)
                    : null,
                fats: fatsController.text.length > 0
                    ? double.parse(fatsController.text)
                    : null,
                carbohydrates: carbohydratesController.text.length > 0
